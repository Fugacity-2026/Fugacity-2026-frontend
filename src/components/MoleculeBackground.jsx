...
export default function MoleculeBackground() {
  ...
  useEffect(() => {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        particles[pIdx].anchorX = (window.innerWidth / (cols + 1)) * (c + 1) + (Math.random() - 0.5) * 30;
        particles[pIdx].anchorY = (window.innerHeight / (rows + 1)) * (r + 1) + (Math.random() - 0.5) * 30;
        pIdx++;
      }
    }
    ...
    const render = () => {
      ...
      currentTempRef.current += (targetTempRef.current - currentTempRef.current) * 0.08;
      ...
      particles.forEach((p) => {
        ...
        if (p.y < 0) {
          if (logicPhase === 'gas') p.y = canvas.height; // Gas wraps cleanly from bottom up
          else p.vy *= -1;
        ...
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.radius + (logicPhase === 'gas' ? 0.5 : 0), 0, Math.PI * 2);
        ctx.fill();
      });
  ...
  return (
    <>
      <canvas ref={canvasRef} className="fixed top-0 left-0 w-full h-full -z-20 pointer-events-none bg-[#020c0d]" />
      
      {/* FIXED HIGH-TECH THERMOMETER OVERLAY GRID */}
      <div className="fixed right-6 top-1/2 -translate-y-1/2 z-50 flex flex-col items-center select-none group">
        ...
        <div className="relative w-7 h-64 bg-slate-900/60 backdrop-blur-sm border-2 border-slate-700/80 rounded-full flex flex-col items-center justify-end p-1 shadow-2xl">
          <div ref={thermometerRef} onMouseDown={handleMouseDown} className="relative w-full h-full rounded-full bg-slate-950/40 cursor-row-resize overflow-hidden flex flex-col justify-end">
            <div className="w-full rounded-b-full transition-all duration-75 ease-out" style={{ height: `${temperature}%`, background: `linear-gradient(to top, #06b6d4, #14b8a6, #f43f5e)` }} />
            <div className="absolute inset-0 flex flex-col justify-between pointer-events-none py-4 px-1 opacity-40 text-[8px] font-mono text-slate-400">
              <div className="flex items-center justify-between"><span>—</span><span>G</span></div>
              <div className="flex items-center justify-between"><span>—</span><span>L</span></div>
              <div className="flex items-center justify-between"><span>—</span><span>S</span></div>
            </div>
          </div>
          <div className="w-6 h-6 rounded-full -mt-2 z-10 transition-colors duration-300 shadow-lg border border-slate-700/50" style={{ backgroundColor: temperature < 35 ? '#06b6d4' : temperature < 75 ? '#14b8a6' : '#f43f5e' }} />
        </div>
        <span className="mt-3 font-mono text-[10px] text-slate-400 tracking-wider">{Math.round(temperature)}°C</span>
      </div>
    </>
  );
}